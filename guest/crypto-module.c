...
{
	debug("Entering vq_has_data");
	debug("Leaving vq_has_data");   // NOTE : Da fuq?
}
...
static int virtcons_probe(struct virtio_device *vdev)
{
	...
	crdev->vq = find_vq(vdev);    // NOTE: This vdev already has a vq? yeap, Hypervisor took care of that
	if (!(crdev->vq)) {
		ret = -ENXIO;
		goto out;
	}
	...
	/* ?? */
	spin_lock_init(& crdev-> device_lock); 

	/**
	 * Grab the next minor number and put the device in the driver's list.
	 **/
	spin_lock_irq(&crdrvdata.lock);   // lock and disable interrupts
	crdev->minor = crdrvdata.next_minor++;
	list_add_tail(&crdev->list, &crdrvdata.devs);
	spin_unlock_irq(&crdrvdata.lock);    // unlock respectively
